...
void City::update(float dt)
{
	...
	for (int i = 0; i < this->map.tiles.size(); ++i)
	{
		...
		else if (tile.tileType == TileType::POWER)
		{
			//Radiate out certain number of layers
			...
			for (int j = 0; j < layers; ++j)
			{
				...
				{
					this->map.tiles[this->shuffledTiles[i] + j].tileVariant = 2;
					this->map.tiles[this->shuffledTiles[i] - j].tileVariant = 2;
					this->map.tiles[this->shuffledTiles[i] + map.width*k + j].tileVariant = 2;
					this->map.tiles[this->shuffledTiles[i] + map.width*k - j].tileVariant = 2;
					this->map.tiles[this->shuffledTiles[i] - map.width*k - j].tileVariant = 2;
					this->map.tiles[this->shuffledTiles[i] - map.width*k + j].tileVariant = 2;
				}	
